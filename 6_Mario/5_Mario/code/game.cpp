...
void Game::initView() {
	view.setSize(1280.f, 640.f);
	view.setCenter(1280.f / 2.f, 640.f / 2.f);
	window->setView(view);
}

void Game::initMenu() {
	if (!font.loadFromFile("img/Minecraft.ttf")) std::cerr << "Erreur font\n";
	if (!menuTx.loadFromFile("img/menu.png")) std::cerr << "Erreur load menu.png\n";
	...
	if(cursorTx.loadFromFile("img/menuCurso.png")) std::cerr << "Erreur load menuCurso.png\n";
...
void Game::initMap() {
	...
	if (!fichier.is_open()) {
		std::cerr << "Impossible d'ouvrir map.txt\n";
		return;
	}
...
void Game::initMonster() {
	if (monsterTx.loadFromFile("img/monster1.png")) std::cerr << "Erreur load mosnter1.png";
	monsters.clear();
	for (int i = 0; i < tileH*tileW; i++) {
		if (level[i] == 10) {
			Monster monster(i);
